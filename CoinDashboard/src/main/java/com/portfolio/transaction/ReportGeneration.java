...
public class ReportGeneration {
	...
	private static  boolean calculateLossDetails = false;
	...
	private void processMainNet(Map<String, ExchangeVO> exchangeList) {
		for(MainnetConfiguration mainData : GetCurrentRate.getMainNetConfigData())
		{
			...
			LOGGER.info("Exchange found for mainnet processing ....");
			ExchangeVO exchangeVO = exchangeList.get(mainData.getExchangeName());
			if(exchangeVO ==null) {
				LOGGER.info("No Data for Exchange :"+exchangeVO);
				continue;
			}
			else {
				LOGGER.info("exchange data "+exchangeVO.getExchangeName());
			}
				...
				if(originalCoin.getCoinName().equalsIgnoreCase( mainData.getCoinName())) {
					if(mainData.isReplaceCoin()) 
					{
						...
						if(mainData.getReplaceCoinName()!= null && !mainData.getReplaceCoinName().equalsIgnoreCase("")) {
							String[] replacementCoin = mainData.getReplaceCoinName().split(",");
							for(String replace: replacementCoin)
							{
								if(replace.equalsIgnoreCase(originalCoin.getCoinName())) {
								...
								{
									replaceCoin =  getXChangeCurrency(exchangeVO.getCoinList(), replace);
									if(replaceCoin== null) {
										...
										replaceCoin.setCoinName(replace);
										replaceCoin.setExchangeName(mainData.getExchangeName());
										replaceCoin.setTotalAmt(originalCoin.getTotalAmt().multiply(mainData.getCoinFactor()));
										exchangeVO.getCoinList().add(replaceCoin);
										//originalCoin.setTotalAmt(ZERO_BIGDECIMAL);
									tradedSymbol.add(replace);
									}
									else {
						...
						}//CHECK FOR REPLACE COIN NAME NULL AND EMPTY
					}
					else {
	...
	{
		//To hold the currentPrice From CoinMarket Cap for Traded symbol
		Map<String,Map<String,Object>> currentMapObject= null;
		//To hold the current Price of Fiat Currency value from currency API layer service
		...
		for (ExchangeVO exchangeVO : exchangeList.values())
		{
			...
			//Get the Current price of traded coins from Xchange for API
			...
			for (ExchangeCurrencyVO exVO : exchangeVO.getCoinList()) 
			{
				...
				for (CurrencyMean currMean : exVO.getCurrency()) 
				{
					...
					//comparing the buy price with Zero
					int compare = currMean.getBuyPrice().compareTo(ZERO_BIGDECIMAL);
					//comparing the buy Volume with Zero
					int compare2 = currMean.getBuyVolume().compareTo(ZERO_BIGDECIMAL);
					//comparing the sell price with Zero
					int compare3 = currMean.getSellPrice().compareTo(ZERO_BIGDECIMAL);
					//comparing the sell volume with Zero
					int compare4 = currMean.getSellVolume().compareTo(ZERO_BIGDECIMAL);
					/*
					 * less than 1  value is lesser than 
					 * Equal to 1  =Compare values are equal here
					 * greater than 1 = Value is equal than
					 * 
					 */
					//Calculating the Buy avg price of coin 
					BigDecimal buyMean = (compare == 1 && compare2 == 1)
							? currMean.getBuyPrice().divide(currMean.getBuyVolume(), 8, RoundingMode.HALF_UP)
									: ZERO_BIGDECIMAL;
							//calculating the Sell avg price of coin
							BigDecimal sellMean = (compare3 == 1 &&  compare4 == 1)
									? currMean.getSellPrice().divide(currMean.getSellVolume(), 8, RoundingMode.HALF_UP)
											: ZERO_BIGDECIMAL;
									//Calculating the  sold  Amount =>( Buy avg price * sold volume)
									BigDecimal soldInvestement = buyMean.multiply(currMean.getSellVolume());
									//Calculating the  invested Amount on coin after sold volume  => Total Buy Price - sold  Amount
									...
									// Percentage calculation on profit Realized
									BigDecimal profitPercentage =ZERO_BIGDECIMAL;
									
									
									BigDecimal netProfit = ZERO_BIGDECIMAL;
									
									BigDecimal netprofitPercentage = ZERO_BIGDECIMAL;
									
									if (compare ==1) {
										// Percentage calculation on profit Realized
										profitPercentage =  profitRealised.divide(currMean.getBuyPrice(), 4, RoundingMode.HALF_UP)
														.multiply(BIG_DECIMAL_100);
											
										//Net Profit Calculation
										netProfit = currMean.getSellPrice().subtract(currMean.getBuyPrice());
												
										//Percentage calculation on net profit									
										netprofitPercentage = netProfit.divide(currMean.getBuyPrice(), 4, RoundingMode.HALF_UP).multiply(BIG_DECIMAL_100);;
									...
									currMean.setNetProfit(netProfit);
									currMean.setNetProfitPercentage(netprofitPercentage);
	...
	{
		Map<String,ExchangeVO> exchangeList = new ConcurrentHashMap<String,ExchangeVO>();
		int lowBP;
		int highBP;
		String exchangeName;
		String coinName;
		...
		LOGGER.fine("Coin \t Currency \t Trade Currency Amt \t Transaction Amt \t Transaction Type \t Total Transaction amt ");
		for(TransactionDetailsVO detailsVO : transactionList){
			...
			if(exchangeList.containsKey(exchangeName))
			{
				...
				if(currencyMean == null)
				{
					...
					if (!coinName.equalsIgnoreCase(currency)) 
					{
						...
						currencyMean.setLowBuyPrice(ZERO_BIGDECIMAL);
						currencyMean.setLowSellPrice(ZERO_BIGDECIMAL);
						currencyMean.setHighBuyPrice(ZERO_BIGDECIMAL);
						currencyMean.setHighSellPrice(ZERO_BIGDECIMAL);
					}
				...
				if(tradeCurrency == null){
					System.out.println("Date ::: "+detailsVO.getDate()+ " Coin :: "+currency);
					tradeCurrency = buildCurrencyInXChange(exchangeName, currency, currencyList);
				}
				if(commissionCurrency != null  && !currency.equalsIgnoreCase(commissionCurrency.trim())) {
					tradeCommission=getXChangeCurrency(currencyList, commissionCurrency.trim());
					if(tradeCommission == null) {
						System.out.println("Date ::: "+detailsVO.getDate()+ " Coin :: "+commissionCurrency.trim());
						tradeCommission = buildCurrencyInXChange(exchangeName, commissionCurrency.trim(), currencyList);
					}
				}
			...
			else
			{
				...
				List<String> tradeCoinList= new CopyOnWriteArrayList<String>();
				List<ExchangeCurrencyVO> exList = new CopyOnWriteArrayList<ExchangeCurrencyVO>();
				List<TransactionCurrency> transList = new CopyOnWriteArrayList<TransactionCurrency>();
				if (!coinName.equalsIgnoreCase(currency)) {
					currencyMean = createNewCurrecnyMean(currency);
					xChangeCurrency.addCurrencyVO(currencyMean);
				...
				if(tradeCurrency == null){
					LOGGER.fine("Date ::: "+detailsVO.getDate()+ " Coin :: "+currency);
				tradeCurrency = buildCurrencyInXChange(exchangeName, currency, exList);
				}
				if(commissionCurrency!= null  && !currency.equalsIgnoreCase(commissionCurrency.trim())) {
					
					if(tradeCommission == null) {
						LOGGER.fine("Date ::: "+detailsVO.getDate()+ " Coin :: "+commissionCurrency.trim());
						tradeCommission = buildCurrencyInXChange(exchangeName, commissionCurrency.trim(), exList);
					}
				}
			...
			if(detailsVO.getTransactionType() == 1 || detailsVO.getTransactionType() ==5 )
			{
				...
				else if(detailsVO.getCommissionType() == VOLUME_BASED)
				{
					...
					if (detailsVO.getCommissionType() == 3) {
						...
						//System.out.println("trade Currency Amt "+tradeCurrencyAmt);
						tradeCurrencyAmt = tradeCurrencyAmt.subtract(transactionAmt.add(commissionRate));
						//System.out.println("trade Currency Amt "+tradeCurrencyAmt);
					...
					{
							tradeCurrencyAmt=tradeCurrencyAmt.subtract(transactionAmt);
							//small conversion process in binance,
							//commission is  calculated with transaction Amt here							
				...
				buyPrice=buyPrice.add(transactionAmt);
				 ...
				 if (currencyMean != null) {
					...
					if (lowBP == 1) {
						...
						if(currencyMean.getLowBuyPrice().compareTo(ZERO_BIGDECIMAL) == 0) {
							currencyMean.setLowBuyPrice(price);
						}
					}
					if (highBP <= 0) {
			...
			else if(detailsVO.getTransactionType() ==2 || detailsVO.getTransactionType() ==6  )
			{
				...
				else if(detailsVO.getCommissionType() == 3 || detailsVO.getCommissionType() == 5) 
				{
					if(detailsVO.getCommissionType() == 3) 
					{
						//System.out.println("before Transaction Amt :"+transactionAmt+","+ tradeCurrencyAmt);
					tradeCurrencyAmt=tradeCurrencyAmt.add(transactionAmt.subtract(commissionRate));	
					//System.out.println(" after Transaction Amt :"+transactionAmt+","+ tradeCurrencyAmt);
				...
				totalAmt = totalAmt.subtract(effectiveVolume);
				 ...
				 if (currencyMean != null) {
					...
					if (lowBP == 1) {
						...
						if(currencyMean.getLowSellPrice().compareTo(ZERO_BIGDECIMAL) == 0) {
							currencyMean.setLowSellPrice(price);
						}
					}
					if (highBP <= 0) {
