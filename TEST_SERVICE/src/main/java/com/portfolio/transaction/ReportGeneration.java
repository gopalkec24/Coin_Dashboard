...
public class ReportGeneration {
	...
	public void loadTradeConfig(){
		...
		nonCryptoList.add("MXN");
		nonCryptoList.add("INR");
		nonCryptoList.add("USD");
	}
	...
	public static void main(String[] args) throws NumberFormatException, IOException{
		...
		for(ExchangeVO exchangeVO : exchangeList.values()) {
			...
			pw.println("Coin Transaction Summary");
			for(TransactionCurrency transVO: exchangeVO.getTransCurrency()) {
				...
				pw.println(",,,,"+transVO.getName()+CSV_DELIMITER+transVO.getBuyTransactionAmount()+CSV_DELIMITER+transVO.getSellTransactionAmount()+CSV_DELIMITER+transVO.getCommissionAmount()+CSV_DELIMITER+transVO.getCurrentInvestAmount()+CSV_DELIMITER+overall.multiply(new BigDecimal("100")));
			}
		}
				...
				for (ExchangeCurrencyVO exVO : exchangeVO.getCoinList()) {
					...
					for (CurrencyMean currMean : exVO.getCurrency()) {
						 ...
						 if(transVO== null) {
							 ...
							 transVO.setCrypto(isCryptoCurrency(currMean.getCurrencyName()));
	...
	private boolean isCryptoCurrency(String coinName) {
